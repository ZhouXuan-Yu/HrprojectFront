...
def _check_duplicate_offer(resume_id, demand_id):
    ...
    Returns True if no duplicate, False if duplicate exists.
    """
    try:
...
def create_offer(data):
    ...
    now = datetime.now()
    offer_no = f"OF{now.strftime('%Y%m%d%H%M%S')}"
    ...
    if resume_id and demand_id and not _check_duplicate_offer(resume_id, demand_id):
        raise AppError('DUPLICATE_OFFER', f'该候选人(resume_id={resume_id})在本需求(demand_id={demand_id})已有进行中的Offer')
    ...
    # Create hire event
    event_no = f"HE{now.strftime('%Y%m%d%H%M%S')}"
    event = HireEvent(
        event_no=event_no,
...
def _create_entry_from_offer(offer):
    ...
    now = datetime.now()
    entry_no = f"EN{now.strftime('%Y%m%d%H%M%S')}"
    entry = Entry(
        entry_no=entry_no,
        event_id=event.id,
...
def create_entry(data):
    ...
    entry = Entry(
        ...
        entry_date=_parse_date(data.get('entryDate')),
    )
    db.session.add(entry)
    db.session.commit()
